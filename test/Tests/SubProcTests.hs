...
exec = runTestsEx
    ...
    , (AsmOutput.suppressWarnsRunAsmDropsCrossStreamWarningPreambleTest, Right "CLI --suppress-warns drops HTCC_ASSEMBLER warning preambles even when the preamble is on stdout and the warning is on stderr in -r mode")
    ...
    , (AsmOutput.suppressWarnsRunAsmPreservesStandalonePreSummaryNoteTest, Right "CLI --suppress-warns preserves standalone HTCC_ASSEMBLER notes that appear before a later warning summary in -r mode")
    ...
    , (AsmOutput.outputFileMultiInputBlockScopeExternObjectConflictTest, Right "CLI -o rejects conflicting block-scope extern object declarations during multi-input merges")
    , (AsmOutput.outputFileMultiInputBlockScopeExternObjectFunctionConflictTest, Right "CLI -o rejects block-scope extern object/function collisions during multi-input merges")
    , (AsmOutput.outputFileMultiInputBlockScopeExternPrototypeVisibilityTest, Right "CLI -o keeps block-scope extern prototype refinements out of later file-scope call checking")
    ...
    , (AsmOutput.outputFileOpenFailurePreservesExistingOutputTest, Right "CLI -o preserves existing outputs when opening the destination fails")
    ...
    , (AsmOutput.runAsmDoesNotWaitOnBackgroundProcessGroupHelpersTest, Right "CLI -r does not wait for unrelated HTCC_ASSEMBLER background helpers that stay in the wrapper process group")
