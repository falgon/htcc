...
exec = runTestsEx
    ...
    , (AsmOutput.stdoutMultiInputImplicitFunctionDefinitionWarningTest, Right "CLI stdout suppresses pre-merge implicit-function warnings once another input provides the declaration")
    ...
    , (AsmOutput.runAsmAcceptsMarkerStrippedFinalOutputTest, Right "CLI -r accepts valid final x86_64-ELF outputs even when wrapped drivers strip htcc's unreferenced marker")
