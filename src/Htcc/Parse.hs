...
data ATree a = ATEmpty -- ^ The empty node 
    | ATNode { atkind :: (ATKind a), atype :: CT.TypeKind, atL :: (ATree a), atR :: (ATree a) } -- ^ `ATKind` representing the kind of node and the two branches `ATree` it has
    deriving Show
    ...
    either (Left . ("Invalid function definition",)) $ \(fndec, st) -> case st of
        ((_, HT.TKReserved "{"):_) -> let args = linesBy ((==HT.TKReserved ",") . snd) $ init $ tail fndec in runST $ do
            ...
            if any isLeft mk then return $ Left $ head $ lefts mk else do
                ...
                return $ second3 (flip (ATNode (ATDefFunc (T.pack fname) $ if null mk then Nothing else Just $ rights mk) CT.CTUndef) ATEmpty) <$> stmt st erat ervar
        _ -> stmt tk atn vars
...
stmt xxs@(cur@(_, HT.TKReserved "{"):_) _ vars = flip (maybe $ Left (internalCE, cur)) (HT.takeBrace "{" "}" xxs) $ -- for compound statement
    either (Left . ("The statement is not closed",)) $ \(scope, ds) -> runST $ do
        ...
        nvars <- readSTRef v
        return $ if any isLeft mk then Left $ head $ lefts mk else Right (ds, ATNode (ATBlock (rights mk)) CT.CTUndef ATEmpty ATEmpty, nvars)
...
unary ((_, HT.TKReserved "*"):xs) at vars = second3 (\lhs -> ATNode ATDeref (maybe CT.CTInt id $ CT.derefMaybe $ atype lhs) lhs ATEmpty) <$> unary xs at vars
...
factor (cur@(_, HT.TKIdent v):xs) _ vars = maybe -- for variables
    (let lvars = LVar v CT.CTInt (if null vars then 8 else offset (head vars) + 8):vars in Right (xs, ATNode (ATLVar (vtype $ head lvars) (offset $ head lvars)) CT.CTInt ATEmpty ATEmpty, lvars))
    -- Left ("undefined variable", cur)
    (\(LVar _ t o) -> Right (xs, ATNode (ATLVar t o) CT.CTInt ATEmpty ATEmpty, vars)) $
